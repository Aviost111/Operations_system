...
int myunzip(char *file) {
    ...
    if (pid == 0) {
        // Child process: Execute gpg command to decrypt the file and write to the temporary file
    ...
    pid = fork();
    if (pid == 0) {
        // Child process: Execute gunzip command, reading from the temporary file
    ...
    if (pid == 0) {
        // Child process: Execute tar command to extract the archive
...
int main(int argc, char **argv) {
    ...
    char *compressedFile = argv[1];

    // Validate file
    if (access(compressedFile, F_OK) != 0) {
        printf("Error: File %s does not exist\n", compressedFile);
        exit(1);
    }

    // Call myunzip
    int status = myunzip(compressedFile);

    if (status == 0) {
        printf("File successfully uncompressed\n");
    } else {
